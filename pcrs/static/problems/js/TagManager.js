...
TagManager.findTagRanges = function(code) {
    ...
    var supported_tags = ['blocked', 'hidden', 'student_code']
    var supported_tags_suffix = ''
    var supported_tags_open = '^[\\s]*\\[('
    var supported_tags_close = '^[\\s]*\\[\\/('

    var tag_lists = {
        'no_tag': []
    };

    var count_tags = supported_tags.length
    for (var i = 0; i < count_tags; i++) {
        tag = supported_tags[i]
        tag_lists[tag] = []
        supported_tags_suffix += tag
        if (i != count_tags - 1) {
            supported_tags_suffix += '|'
        }
    }
    supported_tags_suffix += ')\\][\\s]*$'
    supported_tags_open = RegExp(supported_tags_open + supported_tags_suffix)
    supported_tags_close = RegExp(supported_tags_close + supported_tags_suffix)
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (match = line.match(supported_tags_open)) {
            tag_depth += 1;
        ...
        } else if (match = line.match(supported_tags_close)) {
            lastTagLine = i;
            tag_depth -= 1;
            var tagList = tag_lists[match[1]];
