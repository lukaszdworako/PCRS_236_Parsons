...
SubmissionWrapper.prototype._getTestcasesCallback = function(data) {
    ...
    if (data['num_attempts'] >= data['max_attempts']) {
        this.wrapperDiv.find('#deadline_msg').remove();
        this.wrapperDiv.find('#alert').after(
            '<div style="margin-top: 20px;" id="deadline_msg" class="red-alert">' +
            'You have exceeded the maximum number of attempts (' + data['max_attempts'] + ') for this problem.<div>');
        return;
    }
