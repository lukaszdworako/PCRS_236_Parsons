...
function find_item(){
    ...
    var search_for_list = searching_for.split(",");
    ...
    for (var index = 0; index < problem_list.length; index ++){
        ...
        if (searching_for == ""){
            ...
            var empty_word = false;
            for (var word_index = 0; word_index < search_for_list.length; word_index++){
                var word = search_for_list[word_index].trim();
                if (word == ""){
                    empty_word = true;
                    tag_counter++;
                }
                else{
                    if ($(problem_list[index]).find('.searchable_content').text().toLowerCase().indexOf(word) != -1){
                        word_counter++;
                    }
            ...
            if (tag_counter == search_for_list.length){
                $(problem_list[index]).show();
            }
            else if (word_counter == search_for_list.length){
                $(problem_list[index]).show();
            }
        }
    }
