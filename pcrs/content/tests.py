...
class TestQuestsPageDatabaseHits(ProtectedViewTestMixin, TransactionTestCase):
    ...
    def test_quests_only(self):
        ...
        for i in range(num_quests):
            c = Container.objects.create(name=str(i), description='c')
            SectionContainer.objects.create(open_on=now(), due_on=now(),
                                            container=c, is_graded=True,
                                            section=self.section,
                                            visibility='open')
        self.assertEqual(num_quests, Container.objects.count())
        self.assertEqual(num_quests, SectionContainer.objects.count())
        with self.assertNumQueries(self.db_hits):
            ...
            for j in range(num_ps):
                ...
                for k in range(num_problems):
                    ...
                    p = Problem.objects.create(name=name, description='d',
                                               visibility='open')
                    ContainerProblem(problem_object=p, container_object=ps)\
                        .save()
