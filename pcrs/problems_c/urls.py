...
urlpatterns = patterns('',
    ...
    url(r'^(?P<pk>[0-9]+)/export$',
        ProgrammingProblemExportView.as_view(model=Problem),
        name='code_problem_export'),
    ...
    url(r'^(?P<problem>[0-9]+)/submit$',
        SubmissionView.as_view(model=Submission,
                               template_name='problems_c/submission.html'),
    ...
    url(r'^editor/run$',
        EditorAsyncView.as_view(model=Submission, pType='c'),
