...
document.getElementById("waitingModal").setAttribute("style","display: block");$.post(call_path,postParams,function(data){if(data['past_dead_line']){alert("This submission is past the deadline!")
...
document.getElementById("waitingModal").setAttribute("style","");},"json").fail(function(jqXHR,textStatus,errorThrown){document.getElementById("waitingModal").setAttribute("style","");console.log(textStatus);});}
...
function prepareGradingTable(div_id,best,past_dead_line,sub_pk,max_score){var gradingTable=$("#"+div_id).find("#table_location");var score=0;var tests=[];gradingTable.empty();if(error_msg!=null){gradingTable.append("<div class='red-alert'>"+error_msg+"</div>");error_msg=null;}
...
$("#"+div_id).find('#tcase_'+div_id+'_'+i+' td.passed').html(smFace.clone());if(language!='c'&&testcaseInput!=null){newRow.append('<td class="debug"><button id="'+
...
else if(language=="c"){var codeObj=removeTags($(all_wrappers[x]).find('#div_id_submission').text());myCodeMirrors[all_wrappers[x].id]=history_code_mirror(language,'text/x-csrc',$(all_wrappers[x]).find("#div_id_submission"),codeObj.source_code,false);highlightCode(all_wrappers[x].id,codeObj.tag_list);}
...
$(window).bind("load",function(){$('.CodeMirror').each(function(i,el){el.CodeMirror.refresh();});});});function executeGenericVisualizer(option,data){var supportedVisualization=['python','c'];if(visualizationSupported()){if(language=='python'){return executePythonVisualizer(option,data);}
else if(language=='c'){return executeCVisualizer(option,data);}}else{alert("No support for visualization available!");return null;}
...
function executeCVisualizer(option,data){switch(option){case"create_visualizer":createVisualizer(data);break;case"gen_execution_trace_params":getExecutionTraceParams(data);break;case"render_data":codeStr=data.codeStr;targetElement=data.targetElement;renderVal(codeStr,targetElement);break;default:return"option not supported";}
function createVisualizer(data){if(errorsInTraceC(data)){changeView("edit-code");}
else{visualizer=createVisualizerC(data);visualizer.updateOutput();}}
function errorsInTraceC(data){var errors_caught=false;if(data.exception){alert(data.exception);errors_caught=true;}
else{trace=data.trace;if(trace.length==0){var errorLineNo=trace[0].line-1;if(errorLineNo!==undefined){mainCodeMirror.focus();mainCodeMirror.setCursor(errorLineNo,0);mainCodeMirror.setLineClass(errorLineNo,null,'errorLine');mainCodeMirror.setOption('onChange',function(){mainCodeMirror.setLineClass(errorLineNo,null,null);mainCodeMirror.setOption('onChange',null);});}
alert("Syntax error, cannot visualize code execution");errors_caught=true;}
else if(trace[trace.length-1].exception_msg){alert(trace[trace.length-1].exception_msg);errors_caught=true;}
else if(!trace){alert("Unknown error.");errors_caught=true;}}
return errors_caught;}
function createVisualizerC(data){var cVisualizer=new ExecutionVisualizer('visualize-code',data,{startingInstruction:0,updateOutputCallback:function(){$('#urlOutput,#embedCodeOutput').val('');},disableHeapNesting:('true'=='true'),drawParentPointers:('true'=='true'),textualMemoryLabels:('true'=='true')});$(document).keydown(function(k){if(k.keyCode==37){if(cVisualizer.stepBack()){k.preventDefault();keyStuckDown=true;}}
else if(k.keyCode==39){if(cVisualizer.stepForward()){k.preventDefault();keyStuckDown=true;}}});$(document).keyup(function(k){keyStuckDown=false;});$(document).scrollTop(0);return cVisualizer;}
