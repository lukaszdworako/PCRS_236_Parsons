...
function createVisualizer(data){if(errorsInTraceC(data)){alert("okkkkkkkkkkk");changeView("edit-code");}
else{visualizer=createVisualizerC(data);visualizer.updateOutput();}}
function errorsInTraceC(data){var errors_caught=false;alert(errors_caught);if(data.exception!=" "){alert(data.exception);errors_caught=true;}
else{trace=data.trace;alert(trace.length);if(trace.length==0){alert("entrou");var errorLineNo=trace[0].line-1;if(errorLineNo!==undefined){mainCodeMirror.focus();mainCodeMirror.setCursor(errorLineNo,0);mainCodeMirror.setLineClass(errorLineNo,null,'errorLine');mainCodeMirror.setOption('onChange',function(){mainCodeMirror.setLineClass(errorLineNo,null,null);mainCodeMirror.setOption('onChange',null);});}
alert("Syntax error, cannot visualize code execution");errors_caught=true;}
else if(!trace){alert("entrou2");alert("Unknown error.");errors_caught=true;}}
alert(errors_caught);return errors_caught;}
