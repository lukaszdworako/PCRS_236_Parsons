...
function createVisualizer(data_debugger,newCode){myCodeMirrors[debugger_id].setValue(newCode);$('#visualizerModal').on('shown.bs.modal',function(){myCodeMirrors[debugger_id].refresh();});$('#previous_debugger').bind('click',{data:data_debugger},function(){for(var i=0;i<data[debugger_index].length;i++){console.log(data[debugger_index][i][0]);console.log(typeof(data[debugger_index][i][0]));myCodeMirrors[debugger_id].addLineClass(parseInt(data[debugger_index][i][0]),'','CodeMirror-activeline-background');$('#debugger_table').append('<tr>'+'<th class="text-nowrap" scope="row">Grid behavior</th>'+'<td>Horizontal at all times</td><td>Collapsed to start, horizontal above breakpoints</td>'+'<td>teste</td>'+'</tr>');}
if(debugger_index>=1){debugger_index--;}});$('#next_debugger').bind('click',function(data){});$('#reset_debugger').bind('click',function(data){});$('#visualizerModal').modal('show');}
