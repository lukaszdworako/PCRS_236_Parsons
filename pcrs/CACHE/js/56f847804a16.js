...
return false;}
function createVisualizer(data,newCode){debugger_index=0;last_stepped_line_debugger=0;myCodeMirrors[debugger_id].setValue(newCode);$('#visualizerModal').on('shown.bs.modal',function(){myCodeMirrors[debugger_id].refresh();});update_debugger_table(data);$('#visualizerModal').modal('show');$('#previous_debugger').bind('click',{data:data},function(){console.log("previous: "+debugger_index);if(debugger_index>=1){debugger_index--;}
