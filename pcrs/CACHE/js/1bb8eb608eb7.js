...
function createVisualizer(data_debugger,newCode){myCodeMirrors[debugger_id].setValue(newCode);$('#visualizerModal').on('shown.bs.modal',function(){myCodeMirrors[debugger_id].refresh();});$('#previous_debugger').bind('click',{data:data_debugger},function(){for(var i=0;i<data[debugger_index].length;i++){console.log(data[debugger_index][i]);myCodeMirrors[debugger_id].addLineClass(1,'','CodeMirror-activeline-background');$('#debugger_table').append('<tr>'+'<th class="text-nowrap" scope="row">Grid behavior</th>'+'<td>Horizontal at all times</td><td>Collapsed to start, horizontal above breakpoints</td>'+'<td>teste</td>'+'</tr>');}
