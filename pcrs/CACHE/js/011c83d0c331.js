...
var testcases=null;var error_msg=null;var code_problem_id=-1;var myCodeMirrors={};var cmh_list={};var debugger_id="";var debugger_index=1;function bindDebugButton(buttonId){$('#'+buttonId).bind('click',function(){var testcaseCode=$('#tcase_'+buttonId).find(".expression_div").text();setTimeout(function(){prepareVisualizer("debug",testcaseCode,buttonId)},250);});}
...
function createVisualizer(data,newCode){myCodeMirrors[debugger_id].setValue(newCode);$('#visualizerModal').on('shown.bs.modal',function(){myCodeMirrors[debugger_id].refresh();});update_debugger_table(data);$('#visualizerModal').modal('show');$('#previous_debugger').bind('click',{data:data},function(){update_debugger_table(data[debugger_index]);if(debugger_index>=1){debugger_index--;}});$('#next_debugger').bind('click',{data:data},function(){update_debugger_table(data[debugger_index]);if(debugger_index>=1){debugger_index++;}});$('#reset_debugger').bind('click',{data:data},function(){debugger_index=1;update_debugger_table(data[debugger_index]);});}
function update_debugger_table(data){console.log(data);$('#debugger_table').empty();for(var i=0;i<data[debugger_index].length;i++){myCodeMirrors[debugger_id].addLineClass(parseInt(data[debugger_index][i][0])-1,'','CodeMirror-activeline-background');$('#debugger_table').append('<tr>'+'<th class="text-nowrap" scope="row">'+data[debugger_index][i][4]+'</th>'+'<td>'+data[debugger_index][i][2]+'</td>'+'<td>'+data[debugger_index][i][3]+'</td>'+'<td>'+data[debugger_index][i][1]+'</td>'+'</tr>');}}
