...
return false;}
function createVisualizer(data,newCode){if(!c_debugger_load){c_debugger_load=true;$('#visualizerModal').on('shown.bs.modal',function(){myCodeMirrors[debugger_id].refresh();});$('#previous_debugger').bind('click',{data:data},function(){console.log("previous: "+debugger_index);if(debugger_index>=1){debugger_index--;}
update_debugger_table(data);});$('#next_debugger').bind('click',{data:data},function(){console.log("next: "+debugger_index);if(typeof(data[debugger_index+1])!='undefined'){debugger_index++;update_debugger_table(data);}});$('#reset_debugger').bind('click',{data:data},function(){debugger_index=0;update_debugger_table(data);});}
