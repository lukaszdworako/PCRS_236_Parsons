...
var testcases=null;var error_msg=null;var code_problem_id=-1;var myCodeMirrors={};var cmh_list={};var debugger_id="";var debugger_index=0;var last_stepped_line_debugger=0;var c_debugger_load=false;function bindDebugButton(buttonId){$('#'+buttonId).bind('click',function(){var testcaseCode=$('#tcase_'+buttonId).find(".expression_div").text();setTimeout(function(){prepareVisualizer("debug",testcaseCode,buttonId)},250);});}
...
function executeCVisualizer(option,data,newCode){switch(option){case"create_visualizer":if(!errorsInTraceC(data)){createVisualizer(data,newCode);}
...
return false;}
function createVisualizer(data,newCode){if(!c_debugger_load){c_debugger_load=true;$('#visualizerModal').on('shown.bs.modal',function(){myCodeMirrors[debugger_id].refresh();});$('#previous_debugger').bind('click',{data:data},function(){if(debugger_index>=1){debugger_index--;}
update_debugger_table(data);});$('#next_debugger').bind('click',{data:data},function(){if(typeof(data[debugger_index+1])!='undefined'){debugger_index++;update_debugger_table(data);}});$('#reset_debugger').bind('click',{data:data},function(){debugger_index=0;update_debugger_table(data);});}
debugger_index=0;last_stepped_line_debugger=0;myCodeMirrors[debugger_id].setValue(newCode);update_debugger_table(data);$('#visualizerModal').modal('show');}
