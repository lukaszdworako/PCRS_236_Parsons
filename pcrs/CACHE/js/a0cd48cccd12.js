...
function update_debugger_table(data){console.log(data);$('#debugger_table').empty();if(debugger_index>0){myCodeMirrors[debugger_id].addLineClass(debugger_index-1,'','CodeMirror-activeline-background');}
myCodeMirrors[debugger_id].addLineClass(last_stepped_line_debugger,'','CodeMirror-studentline-background');for(var i=0;i<data[debugger_index].length;i++){$('#debugger_table').append('<tr>'+'<th class="text-nowrap" scope="row">'+data[debugger_index][i][4]+'</th>'+'<td>'+data[debugger_index][i][2]+'</td>'+'<td>'+data[debugger_index][i][3]+'</td>'+'<td>'+data[debugger_index][i][1]+'</td>'+'</tr>');last_stepped_line_debugger=parseInt(data[debugger_index][i][0]-1);}}
