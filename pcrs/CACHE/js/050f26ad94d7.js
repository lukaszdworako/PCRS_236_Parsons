...
function createVisualizer(data_debugger,newCode){console.log(data);myCodeMirrors[debugger_id].setValue(newCode);$('#visualizerModal').on('shown.bs.modal',function(){myCodeMirrors[debugger_id].refresh();});$('#previous_debugger').bind('click',function(data_debugger){console.log(data_debugger);var tbl_values=data_debugger[debugger_index];console.log(tbl_values);$('#debugger_table').append('<tr>'+'<th class="text-nowrap" scope="row">Grid behavior</th>'+'<td>Horizontal at all times</td><td>Collapsed to start, horizontal above breakpoints</td>'+'<td>teste</td>'+'</tr>');});$('#next_debugger').bind('click',function(data){});$('#reset_debugger').bind('click',function(data){});$('#visualizerModal').modal('show');}
