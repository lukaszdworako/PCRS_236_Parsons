...
var testcases=null;var error_msg=null;var code_problem_id=-1;var myCodeMirrors={};var cmh_list={};var debugger_id="";function bindDebugButton(buttonId){$('#'+buttonId).bind('click',function(){var testcaseCode=$('#tcase_'+buttonId).find(".expression_div").text();setTimeout(function(){prepareVisualizer("debug",testcaseCode,buttonId)},250);});}
...
div_id+"_"+i+'" class="debugBtn" type="button"'+' data-toggle="modal" data-target="#visualizerModal">Trace</button></td>');bindDebugButton(div_id+"_"+i);}
...
else if(language=="c"){var codeObj=removeTags($(all_wrappers[x]).find('#div_id_submission').text());myCodeMirrors[all_wrappers[x].id]=history_code_mirror(language,'text/x-csrc',$(all_wrappers[x]).find("#div_id_submission"),codeObj.source_code,false);debugger_id=all_wrappers[x].id+1;console.log(debugger_id);myCodeMirrors[all_wrappers[x].id+1]=history_code_mirror(language,'text/x-csrc',$("#id_preview_code"),codeObj.source_code,false);myCodeMirrors[all_wrappers[x].id+1].addLineClass(13,'','CodeMirror-activeline-background');highlightCode(all_wrappers[x].id,codeObj.tag_list);}
...
function createVisualizer(data){console.log(source_code);console.log(language);console.log(language_version);if(cmh_list[id_preview_code_field]!=null){cmh_list[id_preview_code_field].setValue(source_code);}else{cmh_list[id_preview_code_field]=history_code_mirror(language,language_version,$('#'),source_code,true);}
